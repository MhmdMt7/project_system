...
export default function App() {
  ...
  const saveEdit = () => {
    let copy = [...phones];
    copy[editIndex] = { ...formData, price: +formData.price, qty: +formData.qty };
    setPhones(copy);
    setIsOpen(false);
  };
  return (
                ...
                <tr key={index}>
                  ...
                  <td>
                    <div className="flex gap-2 items-center">
                      <button onClick={() => increaseQty(index)} className="btn btn-success">+</button>
                      <span>{el.qty}</span>
                      <button onClick={() => decreaseQty(index)} className="btn btn-secondary">-</button>
                    </div>
                  </td>
                  <td>{el.price * el.qty}</td>
                  <td className="flex gap-5">
                    <button onClick={() => deletePhone(index)} className="btn btn-error">
                      Del
                    </button>
                    <button onClick={() => openModal(index)} className="btn btn-warning">
                      Up
                    </button>
                  </td>
                </tr>
        ...
        {isOpen && (
          <div className="fixed inset-0 bg-zinc-800 bg-opacity-50 flex justify-center items-center">
            <div className="bg-amber-400 p-6 rounded-2xl shadow-xl w-96">
              <h2 className=" text-xl text-neutral-950 font-bold mb-4 ">Update Phone</h2>
